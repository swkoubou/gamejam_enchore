(function (global) {
    var TouchScene = Class.create(Scene, {
        ...
        addAllNodes: function () {
            ...
            _.forEach(this.girls, function (girl) {
                if (girl.enabled) {
        ...
        removeAllNodes: function () {
            ...
            this.removeChild(this.activeGirlGroup);
            this.removeChild(this.heartGaugeGroup);
            this.removeChild(this.gacha);
            this.removeChild(this.girlTalkLabel);
            this.removeChild(this.touchHeartGroup);
            _.forEach(this.girls, function (girl) {
                this.removeChild(girl.uiSprite);
            }.bind(this));
        },
        ...
        replaceCostume: function (girl) {
            ...
            var beforeActiveGirl = this.status.activeGirl;
            if (beforeActiveGirl) {
                this.activeGirlGroup.removeChild(beforeActiveGirl.mainSprite);
            }
            this.status.activeGirl = girl;
            this.activeGirlGroup.addChild(this.status.activeGirl.mainSprite);

            // OLD_SWIMSUIT_R18の時だけの特別処理
            if (girl.girlType === Girl.TYPE.OLD_SWIMSUIT_R18) {
                // 左に寄せて、余計なものは非表示
                this.status.activeGirl.mainSprite.x = 0;
                this.removeAllNodes();
                this.addChild(this.sidebar);
                this.addChild(this.activeGirlGroup);
                _.forEach(this.girls, function (girl) {
            ...
            } else if (beforeActiveGirl.girlType === Girl.TYPE.OLD_SWIMSUIT_R18) {
                // 元に戻す
                this.removeAllNodes();
                this.addAllNodes();
            }
