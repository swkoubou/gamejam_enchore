(function (global) {
    var TouchScene = Class.create(Scene, {
        initialize: function (status, girls) {
            ...
            gacha.image = assets[PATH.GACHA.MAIN];
            gacha.x = 0;
            gacha.y = 300;
            ...
            // active girl の表示要グループ
            var activeGirlGroup = new Group();
            activeGirlGroup.addChild(status.activeGirl.mainSprite);

            // 表示する順に追加
            this.addChild(sidebar);
            this.addChild(activeGirlGroup);
            ...
            _.forEach(girls, function (girl) {
                girl.uiSprite.addEventListener('touchend', function () {
                    ...
                    if (status.activeGirl) {
                        activeGirlGroup.removeChild(status.activeGirl.mainSprite);
                    }
                    status.activeGirl = girl;
                    activeGirlGroup.addChild(status.activeGirl.mainSprite);
                });
            });
